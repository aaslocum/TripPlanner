...
<template>
  <header class="fixed top-0 left-0 right-0 h-14 md:h-16 z-50 flex items-center px-3 md:px-6"
    ...
    <button @click="emit('toggle-menu')" class="md:hidden p-2 -ml-1 mr-1 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800">
    ...
    <div class="flex items-center gap-2 md:gap-3">
      <svg class="w-6 h-6 md:w-7 md:h-7 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <h1 class="hidden md:block text-lg font-semibold text-gray-900 dark:text-gray-100">Trip Planner</h1>
    </div>
    ...
    <div class="ml-auto flex items-center gap-2 md:gap-3">
      ...
      <button
        ...
        class="hidden md:block text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 font-medium px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      >
        Impersonate
      </button>
      ...
      <button
        @click="toggle"
        class="hidden md:block p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
      ...
      <div v-if="authStore.activeUser" class="flex items-center gap-2">
        ...
        <span class="hidden md:inline text-sm text-gray-700 dark:text-gray-300">{{ authStore.activeUser.first_name }}</span>
      </div>
    </div>
  </header>
