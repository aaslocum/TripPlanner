...
<template>
  <div>
    <div class="flex items-center justify-between mb-6 gap-3">
      ...
      <div v-if="!showForm">
        <button @click="openAddForm" class="bg-cat-eats text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-cat-eats-hover transition-colors">
          + Add Restaurant
        </button>
      </div>
      <div v-else class="flex items-center gap-2 flex-wrap justify-end">
        <button @click="saveEat" class="bg-cat-eats text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-cat-eats-hover transition-colors">
    ...
    <div v-if="showForm" class="bg-surface dark:bg-dark-surface rounded-xl shadow-sm border border-warm-200 dark:border-dark-border p-6 mb-6">
        ...
        <div class="relative">
          ...
          <div v-if="showDropdown && predictions.length > 0" class="absolute z-10 w-full mt-1 bg-surface dark:bg-dark-raised border border-warm-200 dark:border-dark-border rounded-lg shadow-lg max-h-64 overflow-y-auto">
            <button
              ...
              class="w-full text-left px-4 py-3 hover:bg-cat-eats-light dark:hover:bg-red-950 transition-colors border-b border-warm-100 dark:border-dark-border last:border-b-0"
            >
      ...
      <div v-if="placeSelected" class="mb-5 pb-5 border-b border-warm-200 dark:border-dark-border bg-cat-eats-light dark:bg-red-950 -mx-6 px-6 py-4">
          ...
          <div class="flex-1 p-3 md:p-4">
            <div class="flex items-start justify-between">
              <div>
                ...
                <p v-if="eat.start_datetime" class="text-xs text-cat-eats dark:text-red-400 mt-1">
              ...
              <div v-if="authStore.isAdmin" class="flex gap-1">
                <button @click="openEditForm(eat)" class="text-warm-400 hover:text-cat-eats dark:hover:text-red-400 transition-colors p-1">
            ...
            <div class="flex items-center gap-2 mt-2">
              ...
              <button v-if="eat.latitude && eat.longitude" @click="viewOnMap(eat)" class="text-xs text-cat-eats dark:text-red-400 hover:underline cursor-pointer">View on Map</button>
              <a v-else-if="eat.source_url" :href="eat.source_url" target="_blank" class="text-xs text-cat-eats dark:text-red-400 hover:underline">View on Maps ↗</a>
            </div>
          </div>
        </div>
      </div>
