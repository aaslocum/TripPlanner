  ...
  <div>
    ...
    <div v-if="Object.keys(itinerary).length" class="space-y-8">
      <div v-for="(events, date) in itinerary" :key="date">
        ...
        <div class="relative ml-4 border-l-2 border-gray-200 pl-6 space-y-4">
          <div v-for="(event, idx) in events" :key="idx" class="relative">
            <!-- Timeline dot -->
            <div class="absolute -left-[31px] w-4 h-4 rounded-full border-2 border-white" :class="eventColor(event.type)"></div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <div class="flex items-center gap-3">
                ...
                <span v-if="event.datetime" class="text-sm text-gray-500">{{ formatTime(event.datetime) }}</span>
                <span v-if="event.end_datetime" class="text-sm text-gray-400">- {{ formatTime(event.end_datetime) }}</span>
              </div>
              <h4 class="mt-2 font-medium text-gray-900">{{ event.title }}</h4>
              <p v-if="event.details?.description" class="text-sm text-gray-600 mt-1">{{ event.details.description }}</p>
